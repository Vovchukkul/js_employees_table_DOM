/* eslint-disable no-shadow */ /* eslint-disable padding-line-between-statements */ "use strict";
...
for(let i = 0; i < table.rows.length; i++)table.rows[i].addEventListener("click", ()=>{
    table.rows[i].classList.toggle("active");
    for(let j = 0; j < table.rows.length; j++)if (j !== i) table.rows[j].classList.remove("active");
});
...
const selectOffice = createSelect("office", "Office: ", [
    "Tokyo",
    "Singapore",
    "London",
    "New York",
...
form.addEventListener("submit", (e)=>{
    ...
    if (form.elements.name.value.length < 4) {
        pushNotification(10, 10, "Title of Error message", "Name must have more than 4 letters", "error");
        return;
    }
    if (form.elements.age.value < 18) {
        pushNotification(10, 10, "Error", "Your age must be higher than 18", "error");
        return;
    } else if (form.elements.age.value > 90) {
        pushNotification(10, 10, "Error", "Your age must be less than 90", "error");
        return;
    }
    const newEmployee = {
    ...
    pushNotification(10, 10, "Success", "New employee has been successfuly added", "success");
    form.reset();
});
